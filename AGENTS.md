...
ChromaUI is a monorepo with a Python backend and a Next.js frontend. The backend in `backend/` serves FastAPI-based APIs and depends on `GEMINI_API_KEY` for Gemini access. The frontend in `frontend/` is the user-facing app, with editor and preview flows built on a Next.js app router structure.
